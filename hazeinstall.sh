...
sudo $pkgman install xorg -y
echo -e "$color -> installing X11 tools..."
sudo $pkgman install xinit x11-xserver-utils xterm x11-apps x11-utils -y
echo -e "$color -> installing HaZe dependencies and build tools..."
sudo $pkgman install build-essential libx11-dev libxft-dev libxinerama-dev libxpm-dev libxext-dev pkg-config x11proto-dev checkinstall -y
